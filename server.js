...
if (!AI_BUILDER_TOKEN) {
  console.error("FATAL: AI_BUILDER_TOKEN environment variable is not set. Set it in your deployment's env vars â never hardcode it in this file.");
  process.exit(1);
}
...
const QUESTION_BANK = {
  "job-applications-drop": {
    ...
    frameworkNotes: "Reference framework (for the interviewer's calibration only, never reveal): five steps â high-level check (seasonality/policy/region/platform) -> break down by cohort -> form assumptions (agree on the metric definition, think from the user's journey) -> test with data (design a metric that would move first per assumption) -> recommend with trade-offs. Model hypotheses: less job supply vs. less job demand, narrowing to companies less willing to post vs. users less willing to apply. A strong candidate names all four hypotheses before eliminating two."
  },
  "video-chat-launch": {
...
app.post("/api/chat", async (req, res) => {
  ...
  if (rateLimited(ip, 40, 5 * 60 * 1000)) {
    return res.status(429).json({ error: "rate_limited", message: "Too many requests â wait a moment and try again." });
  }
...
app.post("/api/evaluate", async (req, res) => {
  ...
  if (rateLimited(ip, 10, 15 * 60 * 1000)) {
    return res.status(429).json({ error: "rate_limited", message: "Too many evaluation requests â wait a moment and try again." });
  }
  const { questionId, turns } = req.body || {};
  const question = questionOr404(res, questionId);
  if (!question) return;
  ...
  const evalSystemPrompt = [
    ...
    "Base every evidence line on something the candidate actually said below â never invent a quote or moment. Keep each line under 160 characters.",
    "",
    "--- RUBRIC ---",
    RUBRIC_TEXT,
    "",
